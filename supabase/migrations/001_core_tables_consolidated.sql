-- =====================================================
-- CONTABILEASE - TABELAS PRINCIPAIS CONSOLIDADAS
-- =====================================================
-- Este script cria as tabelas principais do sistema
-- Resolve conflitos dos scripts originais 4 e 5
-- Versão: 1.0.0
-- Data: 2025-01-27
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE trigger_name = 'trg_countries_updated_at' 
...
-- Constraints para contracts (com verificação de existência)
DO $$
BEGIN
    -- Contract value positive check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_contract_value_positive' 
    ...
    -- Contract term positive check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_contract_term_positive' 
        AND table_name = 'contracts'
    ) THEN
    ...
    -- Interest rate valid check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_interest_rate_valid' 
        AND table_name = 'contracts'
    ) THEN
    ...
    -- Payment frequency valid check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_payment_frequency_valid' 
        AND table_name = 'contracts'
    ) THEN
    ...
    -- Payment timing valid check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_payment_timing_valid' 
        AND table_name = 'contracts'
    ) THEN
    ...
    -- Lease classification valid check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_lease_classification_valid' 
        AND table_name = 'contracts'
    ) THEN
    ...
    -- Contract type valid check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_contract_type_valid' 
        AND table_name = 'contracts'
    ) THEN
    ...
    -- Approval status valid check
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_approval_status_valid' 
        AND table_name = 'contracts'
    ) THEN
...
-- Índices para contracts (usando CONCURRENTLY para melhor performance)
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_contract_type ON contracts(contract_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_approval_status ON contracts(approval_status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_lease_dates ON contracts(lease_start_date, lease_end_date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_contract_value ON contracts(contract_value);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_payment_frequency ON contracts(payment_frequency);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE trigger_name = 'trg_contracts_updated_at' 
...
-- Índices para tabelas relacionadas (usando CONCURRENTLY)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contract_variable_payments_contract_id ON contract_variable_payments(contract_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contract_renewal_options_contract_id ON contract_renewal_options(contract_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contract_documents_contract_id ON contract_documents(contract_id);

-- Triggers para updated_at
DO $$
BEGIN
    -- Variable payments trigger
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE trigger_name = 'trg_contract_variable_payments_updated_at' 
        AND event_object_table = 'contract_variable_payments'
    ) THEN
    ...
    -- Renewal options trigger
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE trigger_name = 'trg_contract_renewal_options_updated_at' 
        AND event_object_table = 'contract_renewal_options'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE trigger_name = 'trg_contract_documents_updated_at' 
        AND event_object_table = 'contract_documents'
    ) THEN
