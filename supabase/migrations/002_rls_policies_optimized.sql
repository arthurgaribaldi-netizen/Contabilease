...
-- Índices para melhorar performance das políticas RLS (usando CONCURRENTLY)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_permissions_lookup ON user_permissions(user_id, resource_id, resource_type, permission, is_active);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_user_id_status ON contracts(user_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contracts_user_id_type ON contracts(user_id, contract_type);
