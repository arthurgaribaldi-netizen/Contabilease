  ...
  const handlePlanSelect = async (plan: SubscriptionPlan) => {
    ...
    try {
      const response = await fetch('/api/stripe/create-checkout-session', {
        ...
        body: JSON.stringify({
          priceId:
            billingCycle === 'monthly' ? plan.stripe_price_id_monthly : plan.stripe_price_id_yearly,
          planName: plan.name,
        }),
      });
    ...
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
      ...
      <div className='text-center mb-12'>
        ...
        <div className='flex items-center justify-center space-x-4 mb-8'>
          <span
            className={`text-sm ${billingCycle === 'monthly' ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}
          >
            Mensal
          </span>
          <button
          ...
          <span
            className={`text-sm ${billingCycle === 'yearly' ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}
          >
            Anual
