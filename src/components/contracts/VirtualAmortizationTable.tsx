...
export default function VirtualAmortizationTable({
  ...
  const fetchAmortizationData = useCallback(
  ...
  const fetchSummary = useCallback(async () => {
    try {
      ...
      logger.error(
        ...
        {
          component: 'virtualamortizationtable',
  ...
  // Load data on mount
  useEffect(() => {
    fetchAmortizationData(currentPage).catch(error => {
      logger.error(
        'Error fetching amortization data:',
        {
          component: 'virtualamortizationtable',
          operation: 'fetchAmortizationData',
        },
        error as Error
      );
    });
    ...
    ({ index, style }: { index: number; style: React.CSSProperties }) => {
      ...
      return (
        <div style={style} className='flex items-center border-b border-gray-200 hover:bg-gray-50'>
          <div className='flex-1 px-6 py-3 text-sm font-medium text-gray-900'>{period.period}</div>
          <div className='flex-1 px-6 py-3 text-sm text-gray-500'>{formatDate(period.date)}</div>
        ...
        <div className='flex justify-between items-center'>
          ...
          {pagination && (
            <div className='flex items-center space-x-2'>
              <span className='text-sm text-gray-500'>{pagination.totalItems} períodos</span>
              {loading && (
